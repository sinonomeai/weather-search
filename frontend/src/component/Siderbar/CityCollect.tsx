...
export const CityCollect = () => {
  ...
  if (!userData) {
    return (
      <div className='h-9/10 w-full'>
        <h1 className='text-2xl text-center font-bold mb-4'>{t("my-collection")}</h1>
        <div className='flex items-center justify-center h-40'>
          <p className='text-gray-400 text-lg'>
            {weatherData ? t("no-data-login") : t("no-data-login-search")}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className='min-h-[360px] w-full '>
      <h1 className='text-[clamp(18px,1vw,20px)] font-bold text-center my-[4px]'>
        {t("my-collection")}
      </h1>
            ...
            <div className='max-w-[150px] min-w-[100px] '>
              <Button
                ...
                <span className='text-[clamp(12px,1vw,14px)]'>
                  {isCollected ? t("isCollected-true") : t("isCollected-false")}
                </span>
              </Button>
            </div>
          </div>
